...
<div x-data="{ pageName: `Edit Anggota Keluarga`}">
...
<div class="mt-6 space-y-6">
    ...
    <form action="{{ route('admin.warga.anggota.update', $anggota->id) }}" method="POST">
        @csrf
        @method('PUT')

        {{-- BOX 1 — INFO KK --}}
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">Kartu Keluarga</h3>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-xs text-gray-400 mb-1">No KK</p>
                    <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $anggota->kartuKeluarga->no_kk }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Alamat</p>
                    <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $anggota->kartuKeluarga->alamat }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] mt-5">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">Data Anggota</h3>
            ...
            <div class="mt-5">
                ...
                >
                    ...
                    <option value="laki_laki" {{ old('jenis_kelamin', $anggota->jenis_kelamin) == 'laki_laki' ? 'selected' : '' }}>Laki-laki</option>
            ...
            <div class="mt-5">
                ...
                >
                    <option value="">Pilih</option>
                    <option value="istri"   {{ old('status_dalam_kk', $anggota->status_dalam_kk) == 'istri'   ? 'selected' : '' }}>Istri</option>
                    <option value="anak"    {{ old('status_dalam_kk', $anggota->status_dalam_kk) == 'anak'    ? 'selected' : '' }}>Anak</option>
                    <option value="lainnya" {{ old('status_dalam_kk', $anggota->status_dalam_kk) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            </div>
            ...
            <div class="mt-5">
                ...
                >
                    <option value="">Pilih</option>
                    <option value="aktif"       {{ old('status_warga', $anggota->status_warga) == 'aktif'       ? 'selected' : '' }}>Aktif</option>
                    <option value="tidak_aktif" {{ old('status_warga', $anggota->status_warga) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                    <option value="pindah"      {{ old('status_warga', $anggota->status_warga) == 'pindah'      ? 'selected' : '' }}>Pindah</option>
                    <option value="meninggal"   {{ old('status_warga', $anggota->status_warga) == 'meninggal'   ? 'selected' : '' }}>Meninggal</option>
                </select>
            </div>
        </div>
            ...
            @else
                ...
                <div class="rounded-xl border border-warning-300 bg-warning-50 p-4 dark:border-warning-500/30 dark:bg-warning-500/10 mb-5">
                    <p class="text-sm font-medium text-warning-700 dark:text-warning-400">
                        ⚠️ Anggota ini belum memiliki akun. Isi email & password untuk membuatkan akun.
                    </p>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
        ...
        <div class="flex justify-end gap-3 mt-5">
            ...
            <a    href="{{ route('admin.warga.show', $anggota->kartuKeluarga->id) }}"
