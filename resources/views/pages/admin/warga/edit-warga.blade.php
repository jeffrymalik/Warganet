...
<div x-data="{ pageName: `Edit Data Warga` }">
...
<div class="mt-6 space-y-6">
    ...
    <form action="{{ route('admin.warga.warga.update', $warga->id) }}" method="POST">
        @csrf
        @method('PUT')

       {{-- BOX 1 — KARTU KELUARGA --}}
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            @if($warga->is_kepala_keluarga)
                {{-- Kepala keluarga — KK tidak bisa diganti, tampilkan info saja --}}
                <div class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-500/30 dark:bg-blue-500/10 mb-4">
                    <p class="text-sm font-medium text-blue-700 dark:text-blue-400">
                        ℹ️ Warga ini adalah <strong>Kepala Keluarga</strong>. Kartu Keluarga tidak dapat diubah dari halaman ini. Gunakan menu <a href="{{ route('admin.warga.edit', $warga->kartuKeluarga->id) }}" class="underline">Edit KK</a> untuk mengubah data KK.
                    </p>
                </div>

                {{-- Tampilkan info KK saja, tanpa dropdown --}}
                <div class="grid grid-cols-2 gap-4 rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-white/[0.02]">
                    <div>
                        <p class="text-xs text-gray-400 mb-1">No. KK</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $warga->kartuKeluarga->no_kk }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Kepala Keluarga</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $warga->nama_lengkap }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-1">Alamat</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $warga->kartuKeluarga->alamat ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-1">No. Rumah</p>
                        <p class="text-sm font-medium text-gray-800 dark:text-white">{{ $warga->kartuKeluarga->no_rumah ?? '-' }}</p>
                    </div>
                </div>

            @else
                {{-- Bukan kepala keluarga — bisa ganti KK --}}
                <p class="text-sm text-gray-400 mb-4">Pilih kartu keluarga untuk warga ini.</p>
                <div x-data="{ selected: '{{ old('kartu_keluarga_id', $warga->kartu_keluarga_id) }}' }">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] mt-5">
            ...
            <div>
                ...
                <input type="text" name="nik" value="{{ old('nik', $warga->nik) }}" placeholder="NIK" inputmode="numeric" maxlength="16"
            ...
            <div class="mt-5">
                ...
                <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $warga->nama_lengkap) }}" placeholder="Nama Lengkap"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
            </div>
            ...
            <div class="mt-5 grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $warga->tempat_lahir) }}" placeholder="Tempat Lahir"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Tanggal Lahir</label>
                    <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', optional($warga->tanggal_lahir)->format('Y-m-d')) }}" onclick="this.showPicker()"
            ...
            <div class="mt-5">
                ...
                <select name="status_dalam_kk" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 pr-11 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    <option value="">Pilih</option>
                    <option value="istri"   {{ old('status_dalam_kk', $warga->status_dalam_kk) == 'istri'   ? 'selected' : '' }}>Istri</option>
                    <option value="anak"    {{ old('status_dalam_kk', $warga->status_dalam_kk) == 'anak'    ? 'selected' : '' }}>Anak</option>
                    <option value="lainnya" {{ old('status_dalam_kk', $warga->status_dalam_kk) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            </div>

            <div class="mt-5">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Pekerjaan</label>
                <input type="text" name="pekerjaan" value="{{ old('pekerjaan', $warga->pekerjaan ?? '') }}" placeholder="Pekerjaan"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
            </div>

            <div class="mt-5">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">No Telepon</label>
                <input type="text" name="no_telepon" value="{{ old('no_telepon', $warga->no_telepon ?? '') }}" placeholder="No Telepon"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
            </div>

            <div class="mt-5">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Pendapatan</label>
                <input type="number" name="pendapatan" value="{{ old('pendapatan', $warga->pendapatan) }}" placeholder="Pendapatan" min="0"
                    class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
            </div>
        ...
        {{-- BOX 3 — AKUN --}}
        <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] mt-5">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">Akun</h3>

            @if($warga->user_id)
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Email</label>
                    <input type="email" name="email" value="{{ old('email', $warga->user->email ?? '') }}" placeholder="Email"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"/>
                </div>
                <div class="mt-5">
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Password <span class="text-gray-400 font-normal">(kosongkan jika tidak ingin diubah)</span>
                    </label>
            ...
            @else
                <div class="rounded-xl border border-warning-300 bg-warning-50 p-4 dark:border-warning-500/30 dark:bg-warning-500/10 mb-5">
                    <p class="text-sm font-medium text-warning-700 dark:text-warning-400">
                        ⚠️ Warga ini belum memiliki akun. Isi email & password untuk membuatkan akun.
                    </p>
                </div>
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
